...
import { renderTaskPilotDocToPdfBuffer } from "@/lib/pdf";
...
describe("PDF Rendering Integration", () => {
  describe("renderTaskPilotDocToPdfBuffer", () => {
    it("should render empty document to valid PDF", async () => {
      ...
      const buffer = await renderTaskPilotDocToPdfBuffer(doc);
  ...
  describe("page options", () => {
    it("should support different page sizes and verify content renders", async () => {
      ...
      const a4Buffer = await renderTaskPilotDocToPdfBuffer(doc, { pageSize: "A4" });
      const letterBuffer = await renderTaskPilotDocToPdfBuffer(doc, { pageSize: "LETTER" });
    ...
    it("should support landscape orientation and verify content", async () => {
      ...
      const portraitBuffer = await renderTaskPilotDocToPdfBuffer(doc, { pageOrientation: "portrait" });
      const landscapeBuffer = await renderTaskPilotDocToPdfBuffer(doc, { pageOrientation: "landscape" });
  ...
  describe("metadata rendering", () => {
    it("should render user mentions with resolved display name", async () => {
      ...
      const buffer = await renderTaskPilotDocToPdfBuffer(doc, { metadata });
      const text = await extractPdfText(buffer);
  ...
  describe("noAssets option", () => {
    it("should render text but skip images when noAssets is true", async () => {
      ...
      const buffer = await renderTaskPilotDocToPdfBuffer(doc, { noAssets: true });
      const text = await extractPdfText(buffer);
