...
The TaskPilot API was audited end-to-end for Django 5.0/5.1/5.2 breaking changes (12 targeted code scans, a full settings audit, a 129-file migration scan, and per-dependency compatibility research). **The application code is already Django 5.2-clean.** Every breaking-change scan came back unaffected.

The migration is therefore **almost entirely a coordinated third-party dependency bump plus verification** — not a code rewrite. Risk is **low–medium** and concentrated in the dependency layer, not in TaskPilot's own code.

**No runtime or infrastructure blockers:**

| Prerequisite | Django 5.2 requires | TaskPilot has                       | Status |
| ------------ | ------------------- | ------------------------------- | ------ |
...
- **Tests:** `pytest` + `pytest-django`, settings module `taskpilot.settings.test`, run with `--reuse-db --nomigrations --strict-markers`.
...
Every Django 5.x removal/behavior-change category was scanned across `taskpilot/` and `templates/`. Summary:
...
- `taskpilot/urls.py` debug-toolbar include guard (django-debug-toolbar 6.0 model requirement).
- `taskpilot/db/migrations/0122_…` state-only M2M `AlterField` (Django 5.1 `through_fields` normalization; no DDL).
- `taskpilot/tests/contract/app/test_authentication.py` autouse cache-reset fixture (fixes 8 pre-existing throttle failures; makes the suite green).
...
| channels                     | 4.1.0   | **4.3.2**                        | 4.2.1       | 4.3 raises `asgiref>=3.9` (auto-resolved) and makes Daphne an optional extra — TaskPilot serves ASGI via uvicorn/gunicorn, so no impact.                                                                    |
...
| django-filter                | 24.2    | **25.2**                         | 25.1        | 25.x removed built-in DRF schema gen (TaskPilot uses drf-spectacular → no impact). 25.2 requires Python ≥3.10 (have 3.12).                                                                                  |
...
| django-redis                 | 5.4.0   | **7.0.0**                        | 6.0.0       | 7.0 renamed zset/hash helper params (not used by TaskPilot) and drops Django 5.0 (irrelevant). **Verify redis-py floor** (`redis==5.0.4`) satisfies 7.0; if not, pin **6.0.0** instead.                     |
...
| celery                               | 5.4.0      | **5.5.3** (not 5.6.x) | **Override:** 5.6 reverted the SQS→urllib3 change; 5.5.3 is the battle-tested line. TaskPilot uses Redis broker. Pulls `kombu>=5.5`. (Celery itself isn't Django-pinned — already works on 5.2.)           |
...
- **Drop pytz → stdlib `zoneinfo`** — touch points: `taskpilot/utils/timezone_converter.py`, `taskpilot/app/views/cycle/base.py`, `taskpilot/app/views/timezone/base.py`, `taskpilot/api/serializers/cycle.py`, the 4 OAuth providers, and `TIMEZONE_CHOICES` in `taskpilot/db/models/{project,user,workspace,cycle}.py`. Note `pytz.common_timezones` has no exact `zoneinfo` equivalent and `tz.localize(dt)` becomes `dt.replace(tzinfo=ZoneInfo(name))`.
