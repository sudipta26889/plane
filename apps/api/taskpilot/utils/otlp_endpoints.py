...
(e.g. https://telemetry.taskpilot.so or https://telemetry.taskpilot.town behind
...
_DEFAULT_OTLP_ENDPOINT = "https://telemetry.taskpilot.so"


def grpc_endpoint_from_url(url: str) -> str:
    """
    Derive gRPC host:port from OTLP_ENDPOINT URL.
    - https://telemetry.taskpilot.so -> telemetry.taskpilot.so:443 (nginx ingress)
    - https://telemetry.taskpilot.town -> telemetry.taskpilot.town:443 (dev)
    - telemetry.taskpilot.so:4317 -> telemetry.taskpilot.so:4317 (scheme-less with port)
    - telemetry.taskpilot.so -> telemetry.taskpilot.so:4317 (scheme-less, default gRPC port)
    ...
    if "://" not in url:
        url = "//" + url
    parsed = urlparse(url)
    host = parsed.hostname or "telemetry.taskpilot.so"
