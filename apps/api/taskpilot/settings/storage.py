...
# Module imports
from taskpilot.utils.exception_logger import log_exception
...
class S3Storage(S3Boto3Storage):
    ...
    def __init__(self, request=None):
        ...
        # Store the public base URL for rewriting presigned URLs
        self._public_base_url = None
        if os.environ.get("USE_MINIO") == "1":
            # Always sign against the internal MinIO endpoint
            # Then rewrite URLs to use the public proxy if a request is available
            if request:
                scheme = "https" if os.environ.get("MINIO_ENDPOINT_SSL") == "1" else request.scheme
                self._public_base_url = f"{scheme}://{request.get_host()}"
    ...
    def generate_presigned_post(self, object_name, file_type, file_size, expiration=None):
        ...
        # Rewrite the URL to use the public proxy if available
        if self._public_base_url and self.aws_s3_endpoint_url:
            response["url"] = response["url"].replace(
                self.aws_s3_endpoint_url, self._public_base_url
            )
    ...
    def generate_presigned_url(
        ...
        # Rewrite the URL to use the public proxy if available
        if self._public_base_url and self.aws_s3_endpoint_url and response:
