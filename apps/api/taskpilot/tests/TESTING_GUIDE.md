# Testing Guide for TaskPilot

This guide explains how to write tests for TaskPilot using our pytest-based testing strategy.
...
```python
import pytest
from taskpilot.api.serializers import MySerializer
...
Smoke tests should be placed in `tests/smoke/` directory and use the `taskpilot_server` fixture to test against a real HTTP server.
...
class TestCriticalFlow:
    @pytest.mark.django_db
    def test_login_flow(self, taskpilot_server, create_user, user_data):
        # Get login URL
        url = f"{taskpilot_server.url}/api/auth/signin/"
...
```python
from taskpilot.tests.factories import UserFactory, WorkspaceFactory
...
python -m pytest -m unit --cov=taskpilot
```
