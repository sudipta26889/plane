...
from taskpilot.utils.ip_address import validate_url
from taskpilot.bgtasks.work_item_link_task import fetch_and_encode_favicon, DEFAULT_FAVICON
from taskpilot.authentication.adapter.base import Adapter
...
class TestWebhookUrlValidation:
    ...
    def test_webhook_url_to_internal_is_rejected(self, ip):
        with patch("taskpilot.utils.ip_address.socket.getaddrinfo") as dns:
...
class TestWebhookPatchContextGuard:
    def _serializer_with_request(self, host):
        from taskpilot.app.serializers import WebhookSerializer
...
class TestWebhookRebinding:
    @patch("taskpilot.utils.url_security.requests.Session")
    @patch("taskpilot.utils.url_security.resolve_and_validate")
    def test_connection_pinned_to_validated_ip(self, mock_resolve, mock_session_cls):
        from taskpilot.utils.url_security import pinned_fetch
...
class TestFaviconRedirect:
    @patch("taskpilot.utils.url_security.requests.Session")
    @patch("taskpilot.utils.url_security.resolve_and_validate")
    @patch("taskpilot.bgtasks.work_item_link_task.socket.getaddrinfo")
...
class TestOAuthAvatarSSRF:
    ...
    @patch("taskpilot.authentication.adapter.base.pinned_fetch_following_redirects")
