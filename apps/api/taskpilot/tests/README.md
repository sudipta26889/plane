...
This directory contains tests for the TaskPilot application. The tests are organized using pytest.
...
TaskPilot has two types of API endpoints:

1. **External API** (`taskpilot.api`):
...
```bash
# Run unit tests
python -m pytest taskpilot/tests/unit/

# Run API contract tests
python -m pytest taskpilot/tests/contract/api/

# Run App contract tests
python -m pytest taskpilot/tests/contract/app/

# Run smoke tests
python -m pytest taskpilot/tests/smoke/
```
...
- `taskpilot_server`: Live Django test server for HTTP-based smoke tests
...
2. Use the correct client fixture based on the API being tested:
   ...
   - For smoke tests with real HTTP, use `taskpilot_server`
...
```bash
python -m pytest --cov=taskpilot --cov-report=term --cov-report=html
```
