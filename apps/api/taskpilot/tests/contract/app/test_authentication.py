...
from taskpilot.authentication.provider.credentials.magic_code import MagicCodeProvider
from taskpilot.authentication.rate_limit import AuthenticationThrottle
from taskpilot.db.models import User
from taskpilot.settings.redis import redis_instance
...
class TestMagicLinkGenerate:
    ...
    def setup_user(self, db):
        ...
        user = User.objects.create(email="user@taskpilot.app")
    ...
    @pytest.mark.django_db
    @patch("taskpilot.bgtasks.magic_link_code_task.magic_link.delay")
    def test_magic_generate(self, mock_magic_link, api_client, setup_user, setup_instance):
        """Test successful magic link generation"""
        url = reverse("magic-generate")

        ri = redis_instance()
        ri.delete("magic_user@taskpilot.app")

        response = api_client.post(url, {"email": "user@taskpilot.app"}, format="json")
        ...
        assert args[0] == "user@taskpilot.app"  # First arg should be the email
    ...
    def test_max_generate_attempt(self, mock_magic_link, api_client, setup_user, setup_instance):
        ...
        for _ in range(4):
            api_client.post(url, {"email": "user@taskpilot.app"}, format="json")
...
class TestSignInEndpoint:
    ...
    def test_password_validity(self, django_client, setup_user, setup_instance):
        ...
        url = reverse("sign-in")
        response = django_client.post(url, {"email": "user@taskpilot.app", "password": "user123"}, follow=True)
    ...
    def test_user_login(self, django_client, setup_user, setup_instance):
        ...
        response = django_client.post(url, {"email": "user@taskpilot.app", "password": "user@123"}, follow=False)
    ...
    def test_next_path_redirection(self, django_client, setup_user, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "user@taskpilot.app", "password": "user@123", "next_path": next_path},
            follow=False,
        )
...
class TestMagicSignIn:
    ...
    def test_expired_invalid_magic_link(self, django_client, setup_user, setup_instance):
        ...
        url = reverse("magic-sign-in")
        response = django_client.post(url, {"email": "user@taskpilot.app", "code": "xxxx-xxxxx-xxxx"}, follow=False)
    ...
    def test_user_does_not_exist(self, django_client, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "nonexistent@taskpilot.app", "code": "xxxx-xxxxx-xxxx"},
            follow=True,
        )
    ...
    def test_magic_code_sign_in(self, mock_magic_link, django_client, api_client, setup_user, setup_instance):
        ...
        response = api_client.post(gen_url, {"email": "user@taskpilot.app"}, format="json")
        ...
        ri = redis_instance()
        user_data = json.loads(ri.get("magic_user@taskpilot.app"))
        token = user_data["token"]

        # Use Django client to test the redirect flow without following redirects
        url = reverse("magic-sign-in")
        response = django_client.post(url, {"email": "user@taskpilot.app", "code": token}, follow=False)
    ...
    def test_magic_sign_in_with_next_path(self, mock_magic_link, django_client, api_client, setup_user, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "user@taskpilot.app", "code": token, "next_path": next_path},
            follow=False,
        )
...
class TestMagicSignUp:
    ...
    def test_user_already_exists(self, django_client, db, setup_instance):
        ...
        User.objects.create(email="existing@taskpilot.app")

        url = reverse("magic-sign-up")
        response = django_client.post(url, {"email": "existing@taskpilot.app", "code": "xxxx-xxxxx-xxxx"}, follow=True)
    ...
    def test_expired_invalid_magic_link(self, django_client, setup_instance):
        ...
        url = reverse("magic-sign-up")
        response = django_client.post(url, {"email": "new@taskpilot.app", "code": "xxxx-xxxxx-xxxx"}, follow=False)
...
class TestMagicSignInVerifyAttempts:
    ...
    EMAIL = "verify-attempts@taskpilot.so"
...
class TestMagicSignUpVerifyAttempts:
    ...
    EMAIL = "signup-verify-attempts@taskpilot.so"
...
class TestAuthenticationThrottle:
    ...
    def test_magic_sign_in_throttled(self, django_client, setup_instance):
        ...
        with patch.object(AuthenticationThrottle, "rate", "2/minute"):
            for _ in range(2):
                response = django_client.post(url, {"email": "throttle@taskpilot.so", "code": "000000"}, follow=False)
                assert response.status_code == 302
    ...
    def test_magic_sign_up_throttled(self, django_client, setup_instance):
        ...
        with patch.object(AuthenticationThrottle, "rate", "1/minute"):
            response = django_client.post(url, {"email": "throttle-up@taskpilot.so", "code": "000000"}, follow=False)
            assert "RATE_LIMIT_EXCEEDED" not in response.url
...
class TestBotUserLoginBlocked:
    ...
    BOT_EMAIL = "bot-login@taskpilot.so"
    HUMAN_EMAIL = "human-login@taskpilot.so"
...
class TestBotUserAdminSignInBlocked:
    ...
    BOT_EMAIL = "admin-bot@taskpilot.so"
    PASSWORD = "Str0ng-Pass!42"
