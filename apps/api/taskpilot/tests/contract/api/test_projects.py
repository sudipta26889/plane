...
from taskpilot.db.models import Project, ProjectMember, State, User, WorkspaceMember
...
def outsider_user(db):
    ...
    outsider = User.objects.create(
        email=f"outsider-{unique_id}@taskpilot.so",
...
class TestProjectListCreateAPIEndpoint:
    ...
    def test_model_activity_not_called_on_rollback(self, api_key_client, workspace, create_user):
        ...
        with (
            mock.patch(
                "taskpilot.api.views.project.State.objects.bulk_create",
                side_effect=forced_error,
            ),
            mock.patch("taskpilot.api.views.project.model_activity") as mocked_activity,
        ):
            response = api_key_client.post(url, payload, format="json")
    ...
    def test_response_still_201_when_broker_dispatch_fails(self, api_key_client, workspace, create_user):
        ...
        with mock.patch("taskpilot.api.views.project.model_activity") as mocked_activity:
