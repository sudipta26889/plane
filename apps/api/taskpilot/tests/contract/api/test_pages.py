...
def other_user(db):
    ...
    user = User.objects.create(
        email=f"other-{unique_id}@taskpilot.so",
...
def member_api_key_client(db, project, other_user):
    ...
    from taskpilot.db.models.api import APIToken
...
class TestPageListCreateAPIEndpoint:
    ...
    @pytest.mark.django_db
    def test_create_page_round_trips_description_json(self, api_key_client, workspace, project, page_data):
        """Test description_json is stored on create and returned unchanged on retrieve"""
        description_json = {
            "type": "doc",
            "content": [
        ...
        with patch("taskpilot.api.views.page.page_transaction"):
            response = api_key_client.post(url, {**page_data, "description_json": description_json}, format="json")

        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["description_json"] == description_json

        detail = api_key_client.get(f"{url}{response.data['id']}/")
        assert detail.status_code == status.HTTP_200_OK
        assert detail.data["description_json"] == description_json

    @pytest.mark.django_db
    def test_create_page_with_labels(self, api_key_client, workspace, project, create_user, page_data):
        """Test labels sent on create are attached and returned"""
        label = Label.objects.create(name="Docs", project=project, workspace=project.workspace, created_by=create_user)
        url = self.get_page_url(workspace.slug, project.id)
        ...
        assert Page.objects.first().labels.count() == 1
...
class TestPageDetailAPIEndpoint:
    ...
    @pytest.mark.django_db
    def test_update_page_round_trips_description_json(self, api_key_client, workspace, project, create_page):
        """Test description_json survives a PATCH and comes back unchanged"""
        description_json = {"type": "doc", "content": [{"type": "paragraph", "content": []}]}
        url = self.get_page_url(workspace.slug, project.id, create_page.id)

        response = api_key_client.patch(url, {"description_json": description_json}, format="json")

        assert response.status_code == status.HTTP_200_OK
        assert response.data["description_json"] == description_json
        assert api_key_client.get(url).data["description_json"] == description_json
    ...
    def test_delete_archived_page(self, api_key_client, workspace, project, create_page):
        ...
        # soft deletes fan out to related rows through the broker
        with patch("taskpilot.db.mixins.soft_delete_related_objects"):
            response = api_key_client.delete(url)
