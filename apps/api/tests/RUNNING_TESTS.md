...
docker compose -f docker-compose-test.yml run --rm api-tests \
  pytest taskpilot/tests/unit -k "test_workspace"

# Single file with verbose output
docker compose -f docker-compose-test.yml run --rm api-tests \
  pytest taskpilot/tests/unit/models/test_workspace.py -vv
```
...
Test-time env overrides live in the compose file itself (`POSTGRES_HOST=test-db`, `REDIS_URL=redis://test-redis:6379/`, `AWS_S3_ENDPOINT_URL=http://test-minio:9000`, `DJANGO_SETTINGS_MODULE=taskpilot.settings.test`). Everything else is inherited from `apps/api/.env`.
