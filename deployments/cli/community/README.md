...
## Installing TaskPilot

Installing taskpilot is a very easy and minimal step process.
...
```
mkdir taskpilot-selfhost

cd taskpilot-selfhost
```

#### For *Docker Compose* based setup

```
curl -fsSL -o setup.sh https://github.com/taskpilot/taskpilot/releases/latest/download/setup.sh
...
```
curl -fsSL -o setup.sh https://github.com/taskpilot/taskpilot/releases/latest/download/swarm.sh
...
Above steps will set you ready to install and start taskpilot services.
...
This will create a folder `taskpilot-app` and will download 2 files inside that
...
This will create a create a folder `taskpilot-app` and will download 2 files inside that
...
> `WEB_URL` - This is default set to `http://localhost`. Change this to the FQDN you plan to use along with LISTEN_HTTP_PORT (eg. `https://taskpilot.example.com:8080` or `http://[IP-ADDRESS]:8080`)

> `CORS_ALLOWED_ORIGINS` - This is default set to `http://localhost`. Change this to the FQDN you plan to use along with LISTEN_HTTP_PORT (eg. `https://taskpilot.example.com:8080` or `http://[IP-ADDRESS]:8080`)
...
You have successfully self hosted `TaskPilot` instance. Access the application by going to IP or domain you have configured it (e.g `https://taskpilot.example.com:8080` or `http://[IP-ADDRESS]:8080`)

---

### Stopping the Server / Remove Stack

In case you want to make changes to `taskpilot.env` variables, we suggest you to stop the services before doing that.
...
In case you want to make changes to `taskpilot.env` variables, without stopping the server or you noticed some abnormalies in services, you can restart the services with `RESTART` / `REDEPLOY` option.
...
### Upgrading TaskPilot Version 

It is always advised to keep TaskPilot up to date with the latest release.
...
By choosing this, it will stop the services and then will download the latest `docker-compose.yaml` and `taskpilot.env`.
...
> It is very important for you to validate the `taskpilot.env` for the new changes.

Once done with making changes in `taskpilot.env` file, jump on to `Start Server`
...
Once done with making changes in `taskpilot.env` file, jump on to `Redeploy Stack`
...
```bash
Backing Up taskpilot-app_pgdata
Backing Up taskpilot-app_redisdata
Backing Up taskpilot-app_uploads

Backup completed successfully. Backup files are stored in /....../taskpilot-app/backup/20240502-1120
```
...
1. Make sure that TaskPilot-CE is installed, started, and then stopped. This ensures that the Docker volumes are created.

1. Download the restore script using the command below. We suggest downloading it in the same folder as `setup.sh`.

   ```bash
   curl -fsSL -o restore.sh https://github.com/taskpilot/taskpilot/releases/latest/download/restore.sh
   chmod +x restore.sh
   ```
...
1. Execute the command below to restore your data.
   ...
   As an example, for a backup folder `/opt/taskpilot-selfhost/taskpilot-app/backup/20240722-0914`, expect the response below:
   ...
   --------------------------------------------
   Found /opt/taskpilot-selfhost/taskpilot-app/backup/20240722-0914/pgdata.tar.gz
   .....Restoring taskpilot-app_pgdata
   .....Successfully restored volume taskpilot-app_pgdata from pgdata.tar.gz

   Found /opt/taskpilot-selfhost/taskpilot-app/backup/20240722-0914/redisdata.tar.gz
   .....Restoring taskpilot-app_redisdata
   .....Successfully restored volume taskpilot-app_redisdata from redisdata.tar.gz

   Found /opt/taskpilot-selfhost/taskpilot-app/backup/20240722-0914/uploads.tar.gz
   .....Restoring taskpilot-app_uploads
   .....Successfully restored volume taskpilot-app_uploads from uploads.tar.gz


   Restore completed successfully.
   ```

1. Start the TaskPilot instance using `./setup.sh start`.

---

### Restore for Commercial Air-Gapped (Docker Compose)

When you want to restore the previously backed-up data on TaskPilot Commercial Air-Gapped version, follow the instructions below.

1. Download the restore script using the command below

   ```bash
   curl -fsSL -o restore-airgapped.sh https://github.com/taskpilot/taskpilot/releases/latest/download/restore-airgapped.sh
   chmod +x restore-airgapped.sh
   ```

1. Copy the backup folder and the `restore-airgapped.sh` to `Commercial Airgapped Edition` server

1. Make sure that TaskPilot Commercial (Airgapped) is extracted and ready to get started. In case it is running, you would need to stop that.
...
1. After restoration, you are ready to start TaskPilot Commercial (Airgapped) will all your previously saved data. 
...
curl -fsSL -o migrate.sh https://raw.githubusercontent.com/taskpilot/taskpilot/master/deploy/selfhost/migration-0.13-0.14.sh
...
To move forward, you would need PREFIX of old setup and new setup. As per above example, `v0132` is the prefix of v0.13.2 and `taskpilot-app` is the prefix of v0.14.0 setup
...
Provide the Destination Volume Prefix : taskpilot-app
```
