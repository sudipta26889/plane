...
update_env_file(){
    ...
    # is already stored in taskpilot.env (survives restarts), or generate a fresh random value on
    # first boot. Never write a publicly-known or placeholder value into taskpilot.env.
    ...
    if [ -z "$SECRET_KEY" ] || [ "$SECRET_KEY" = "$_insecure_sk" ] || [ "$SECRET_KEY" = "$_placeholder_sk" ]; then
        local _stored_sk
        _stored_sk=$(grep "^SECRET_KEY=" taskpilot.env 2>/dev/null | cut -d'=' -f2-)
    ...
    if [ -z "$LIVE_SERVER_SECRET_KEY" ] || [ "$LIVE_SERVER_SECRET_KEY" = "$_insecure_lssk" ] || [ "$LIVE_SERVER_SECRET_KEY" = "$_placeholder_lssk" ]; then
        local _stored_lssk
        _stored_lssk=$(grep "^LIVE_SERVER_SECRET_KEY=" taskpilot.env 2>/dev/null | cut -d'=' -f2-)
